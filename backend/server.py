...
def map_bus_db_to_listing(bus_data: dict) -> dict:
    ...
    result['cenaBrutto'] = bus_data.get(
        'cenaBrutto') or bus_data.get('price_pln')
    ...
    result['kubatura'] = bus_data.get(
        'kubatura') or bus_data.get('engine_displacement_cc')
    result['typNadwozia'] = bus_data.get(
        'typNadwozia') or bus_data.get('body_type')
    result['opis'] = bus_data.get('opis') or bus_data.get('description_html')

    # Zabezpieczenia dla list i obiektów (zapobiega crashom "Cannot read
...
async def create_listing(listing_data: ListingCreate):
    ...
    try:
        ...
        # Twarde domyślne wartości dla kolumn bazy, aby Postgres nie odrzucił
        # zapisu
        defaults = {
            'marka': 'Inna',
            'model': 'Inny',
...
async def update_listing(listing_id: str, listing_update: ListingUpdate):
    ...
    try:
        ...
        update_dict = {
            k: v for k,
            v in listing_update.dict().items() if v is not None}
...
async def delete_listing(listing_id: str):
    ...
    listing = supabase.table('buses').select(
        'zdjecia, zdjecieGlowne').eq('id', listing_id).execute()
    ...
    try:
        ...
        try:
            ...
            supabase.storage.from_("buses").upload(
                path, contents, file_options={
                    "content-type": file.content_type})
            public_url = supabase.storage.from_("buses").get_public_url(path)
            return {"success": True, "url": public_url}
        except Exception as e:
            logging.warning(
                f"Supabase upload failed, falling back to local: {e}")
    ...
    for file in files:
        try:
            if file.content_type not in ALLOWED_TYPES:
                errors.append(
                    f"{file.filename}: Niewłaściwy format (Tylko JPG/PNG/WEBP/AVIF)")
                continue
            ...
            try:
                ...
                public_url = supabase.storage.from_(
                    "buses").get_public_url(path)
                uploaded_urls.append(public_url)
            except Exception as supabase_error:
                logging.warning(
                    f"Supabase upload failed, using local fallback: {supabase_error}")
                local_path = UPLOADS_DIR / "buses"
                local_path.mkdir(parents=True, exist_ok=True)
...
async def scrape_otomoto_endpoint(request: OtomotoScrapeRequest):
    ...
    try:
        ...
        if response.status_code in [403, 401, 429]:
            raise Exception(
                "Otomoto zablokowało zapytanie (Ochrona Cloudflare).")
        ...
        def extract_value(key, label):
            ...
            m2 = re.search(
                label +
                r'[^>]*?>([a-zA-Z0-9\sęóąśłżźćńĘÓĄŚŁŻŹĆŃ]+)<',
                html)
            if m2:
                val = m2.group(1).strip()
                if len(val) < 40 and val != label:
                    return val
            return None
        ...
        data["krajPochodzenia"] = extract_value(
            "country_origin", "Kraj pochodzenia")

        data["bezwypadkowy"] = extract_value(
            "no_accident", "Bezwypadkowy") == "Tak"
        data["serwisowanyWAso"] = extract_value(
            "service_record", "Serwisowany w ASO") == "Tak"
        data["maNumerRejestracyjny"] = extract_value(
            "registered", "Zarejestrowany w Polsce") == "Tak"
        ...
        try:
            ...
            iframe = soup.find(
                'iframe', src=regex_yt.compile(r'youtube\.com|youtu\.be'))
            if iframe:
                yt_link = iframe.get('src', '')

            # Opcja 2: Przeszukanie ukrytych stanów strony (Otomoto często
            # trzyma tam wideo)
            if not yt_link:
                yt_match = regex_yt.search(
            ...
            if yt_link:
                ...
                # Przypisujemy pod kilkoma najpopularniejszymi nazwami
                # zmiennych:
            ...
            p_elem = soup.select_one('[data-testid="ad-price-container"] h3') or soup.find(
                'h3', class_=lambda c: c and 'price' in str(c).lower())
            if not p_elem:
        ...
        desc_elem = soup.select_one(
            '[data-testid="ad-description"]') or soup.select_one('.offer-description__description')
        if desc_elem:
        ...
        if "diesel" in fuel:
            data["paliwo"] = "Diesel"
        elif "benz" in fuel or "petrol" in fuel:
            data["paliwo"] = "Benzyna"
        elif "elektry" in fuel or "electric" in fuel:
            data["paliwo"] = "Elektryczny"
        elif "hybry" in fuel:
            data["paliwo"] = "Hybryda"

        gb = str(data.get("skrzynia", "")).lower()
        if "auto" in gb:
            data["skrzynia"] = "Automatyczna"
        elif "man" in gb:
            data["skrzynia"] = "Manualna"
...
async def get_stats():
    if not supabase:
        return {
            "total": len(MOCK_BUSES), "wyrozniowane": sum(
                1 for b in MOCK_BUSES if b.get('wyrozniowane')), "nowe": sum(
                1 for b in MOCK_BUSES if b.get('nowosc')), "flotowe": sum(
                1 for b in MOCK_BUSES if b.get('flotowy'))}
    response = supabase.table('buses').select('*').execute()
    buses = response.data
    return {
        "total": len(buses),
...
async def get_opinions():
    ...
    response = supabase.table('opinions').select(
        '*').eq('wyswietlaj', True).execute()
...
@api_router.put("/admin/opinions/{op_id}",
                dependencies=[Depends(admin_required)])
async def update_opinion(op_id: str, op: OpinionUpdate):
    data = {k: v for k, v in op.dict(by_alias=True).items() if v is not None}
    if data:
        resp = supabase.table('opinions').update(
            data).eq('id', op_id).execute()
        return resp.data[0]
    return {}


@api_router.delete("/admin/opinions/{op_id}",
                   dependencies=[Depends(admin_required)])
                ...
                if bus.get('data_sprzedazy'):
                    try:
                        ...
                        if datetime.now(timezone.utc) - sell_date > timedelta(days=5):
                            ...
                            if paths:
                                supabase.storage.from_("buses").remove(
                                    [f"buses/{p}" for p in paths])
                    except BaseException:
                        pass
            else:
                is_missing = False
                if bus.get('vin') and len(bus.get('vin')) > 5:
                    if bus.get('vin') not in active_vins:
                        is_missing = True
                else:
