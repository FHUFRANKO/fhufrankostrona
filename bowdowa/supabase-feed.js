...
/* Sekcja listy po nagłówku */
function findListingSection(){
  const RE = /(wyroznione busy|wyróżnione busy|najnowsze oferty|busy i samochody dostawcze|ogloszenia|ogłoszenia)/i;
  const heads = Array.from(document.querySelectorAll("main h1, main h2, main h3")).filter(h=>RE.test(h.textContent||""));
...
function hasManyDetails(node){
  ...
  for(const b of btns){
    ...
    if(t.includes("szczegoly")||t.includes("szczegol")||t.includes("szczeg")) cnt++;
...
function findMockContainers(section, heading){
  const out=[];
  const cand = Array.from(section.querySelectorAll("section,div,ul"));
  for(const c of cand){
    if(c.id==="supabase-feed-root") continue;
    if(heading.compareDocumentPosition(c) & Node.DOCUMENT_POSITION_FOLLOWING){
      const cs=getComputedStyle(c);
      const looksGrid = cs.display==="grid"||cs.display==="flex";
      if(looksGrid && c.children.length>=3 && hasManyDetails(c)) out.push(c);
    }
  }
...
function pluralizeBus(n){ const m10=n%10, m100=n%100; if(n===1)return"1 bus"; if(m10>=2&&m10<=4&&!(m100>=12&&m100<=14))return`${n} busy`; return`${n} busów`; }
function setCounterNear(heading, n){
  const row=heading.parentElement;
  row.querySelectorAll(".supabase-count-badge").forEach(x=>x.remove());
  Array.from(row.querySelectorAll("span,div,small,strong,b")).forEach(x=>{
    const t=(x.textContent||"").trim();
    if(/\d+\s*bus(ów|y)?/i.test(t)) x.style.display="none";
  });
...
/* CSS z !important do trwałego ukrycia */
function ensureHideCSS(){
  if(document.getElementById("supabase-hide-style")) return;
  const st=document.createElement("style");
  st.id="supabase-hide-style";
  st.textContent='[data-supabase-hidden="1"]{display:none !important;}';
  document.head.appendChild(st);
}

/* twarda podmiana w sekcji */
async function replaceInSection(){
  ...
  // znajdź i usuń/ukryj mockowe kontenery (kilka razy + observer)
  const hidePass=()=>findMockContainers(section, heading).forEach(m=>{
    m.setAttribute("data-supabase-hidden","1");
    // ostatecznie — jeśli wracają — usuń całkiem:
    if(m.parentElement) m.parentElement.removeChild(m);
  });
  hidePass(); setTimeout(hidePass,150); setTimeout(hidePass,400); setTimeout(hidePass,800);

  const mo=new MutationObserver(()=>hidePass());
...
function onRoute(){
  ...
  if(p==='/' || /^\/ogloszenia(\/|$)/.test(p)){
    ...
    const obs=new MutationObserver((m,o)=>{ const ctx=findListingSection(); if(ctx){ replaceInSection(); o.disconnect(); } });
